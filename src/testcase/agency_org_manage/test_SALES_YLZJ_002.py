...
class Test_SALES_YLZJ_002():
    ...
    def test_001(self, channel, contractType, contract_no):
        ...
        Test_SALES_YLZJ_002.msg = {"channel": channel, "contract_no": text.split("：")[1]}
        # 关闭
        self.ACR.close_button_ty()
        info("中介机构续签-审批")
        self.MAOA.switch_to_window()
        info("中介机构审批页")
        self.MAOA.into_page(Test_SALES_YLZJ_002.msg["channel"])
        info("查询合同号：{}".format(Test_SALES_YLZJ_002.msg["contract_no"]))
        self.MAOA.query(Test_SALES_YLZJ_002.msg["contract_no"])
        ...
        info("提交任务")
        self.ACRA.submit_interaction("中介机构续签-ui测试")
