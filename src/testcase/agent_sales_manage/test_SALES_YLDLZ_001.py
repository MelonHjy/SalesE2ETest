...
class Test_YLDLZ_001():
    ...
    def test_YLDLZ_001_basemag(self, userName, idCard, mobile, group, rolecode, nation, visage, culture):
        ...
        self.appointment.get_sex(self.appointment_and_dismissal.get_sex_by_idCard(idCard))
        self.appointment.assertEqual("验证出生日期是否与身份证匹配", self.appointment_and_dismissal.get_birthday(),
    ...
    def test_YLDLZ_001_assert(self, name, id_cards, sjjg, group):
        ...
        self.appointment.click(
            self.appointment.wait_until_el_xpath(self.appointment.xstdjlpryjp))
        self.appointment.switch_to_window()
        self.appointment.maximize_window()
        self.appointment.prepare_save_commit()
        self.appointment.submit_process()
        # 切换到‘代理制销售人员代码管理’页面
        self.appointment.switch_to_window()
